...
class OneToOneDataset(Dataset):
    ...
    def __init__(self, cfg: configuration.CFG, p_df: pd.DataFrame, s_df: pd.DataFrame) -> None:
        ...
        self.content = torch.from_numpy(s_df.content.to_numpy())  # target value named 'content'
        self.wording = torch.from_numpy(s_df.wording.to_numpy())  # target value named 'wording'
    ...
    def __getitem__(self, item: int) -> tuple[dict, Tensor, Tensor]:
        ...
        content, wording = self.content[item], self.wording[item]
